  ...
  //                      + " tag.ReadCount:" + tag.ReadCount.ToString());
            ...
            else
            {
                ...
                else
                {
                    ...
                    newtag.Phase = tag.Phase;
                    newtag.Frequency = tag.Frequency;
                    //Console.Write(tag.EPCString);
                    if (DAL.CommonPayRecordDAL.queryByCardID(tag.EPCString.ToString().Trim()) != null)
                    {
                        MessageBox.Show("卡号为" + tag.EPCString + "的车辆经过");

                    }      

                    //对操作层进行操作
                    DAL.CommonPayRecordDAL.insertTimebyCardID(tag.EPCString.ToString().Trim());                  
                    //MessageBox.Show(tag.EPCString.ToString().Trim());
        ...
        public void disconnect1(Exception ex)
        {
...
//                    int ee = Environment.TickCount;
 ...
 //                       Console.WriteLine("Rssi: " + read.Rssi);
  ...
  //          Debug.WriteLine("5555555555555555");
        ...
        public void connect(string ip, string duankou)
        {
            ...
            cbbreadertype.SelectedItem = duankou;
 ...
 /*
        ...
        public void start(bool ants1, bool ants2, bool ants3, bool ants4)
        {
            bool[] ants = { ants1, ants2, ants3, ants4 };
            this.cbpotl6b.Checked = false;
            this.cbpotlgen2.Checked = true;
            this.cbpotlipx256.Checked = false;
            this.cbpotlipx64.Checked = false;
            if (rParms.readertype != ReaderType.MT_A7_16ANTS)//rparms:读取参数
            {
                if (readThread != null)
                {
                    isInventory = false;
                    readThread.Join();//停止主线程，先运行读取线程
                }


                List<AntAndBoll> selants = AntsValid(ants);//判断是否勾选天线
                if (selants.Count == 0)
                {
                ...
                //for (int i = 0; i < selants.Count; ++i)//判断天线是否链接
                //{
                ...
                List<int> antsExe = new List<int>();//设置天线
                for (int i = 0; i < selants.Count; ++i)
                {
                    antsExe.Add(selants[i].antid);
                }

                if ((!this.cbpotl6b.Checked) && (!this.cbpotlgen2.Checked) && (!this.cbpotlipx64.Checked)//判断是否勾选协议
                ...
                List<SimpleReadPlan> readplans = new List<SimpleReadPlan>();//设置读取协议
            ...
            readThread.Start();
            MessageBox.Show("固定式读写器"+"连接成功  connect time："+DateTime.Now+"\n连接地址:"+this.tbip.Text.Trim());
