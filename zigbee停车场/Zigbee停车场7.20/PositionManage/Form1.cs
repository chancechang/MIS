...
namespace PositionManage
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            textBox2.Text = CardID;
            textBox3.Text = placeID;
            //初始化地图
            ...
            //初始化IP和端口信息
            //this.tbIP.Text = Program.GetAppSettingValue("DefaultIP", "192.168.0.197");
            //this.tbPort.Text = Program.GetAppSettingValue("DefaultPortNo", "20108");
            ...
            ImportSampleData();
            openConnect("192.168.0.197", "20108");
            INit();
            Position();
            //btnConnect_Click(new object(), new EventArgs());
            // btnInit_Click(new object(), new EventArgs());
            //btnPosition_Click(new object(), new EventArgs());

        }

        #region 连接&断开
        public void openConnect(string tpIP, string tpPort)
        {
            System.Net.IPAddress ipaddr = null;
            if (!System.Net.IPAddress.TryParse(tpIP, out ipaddr))
            {
            ...
            int iPort = 0;
            if (!int.TryParse(tpPort, out iPort) || iPort <= 0)
            {
            ...
            Program.SetAppSettingValue("DefaultIP", tpIP);
            Program.SetAppSettingValue("DefaultPortNo", tpPort);
        ...
        private void btnConnect_Click(object sender, EventArgs e)
        {
            ...
            Program.SetAppSettingValue("DefaultIP", "192.168.0.197");
            Program.SetAppSettingValue("DefaultPortNo", "20108");
        ...
        private void DealData(UDPData data)
        {
            ...
            VoidDelegate dShow = new VoidDelegate(delegate
            {
                ...
                if (!this.panMap.Controls.ContainsKey("uc" + data.TagNo))
                {
                    ...
                    ucPoint.PointColor = Color.LightBlue;
                ...
                if (a == 1)
                {
                    ucPoint.Location = new Point(50, 115);

                }
                else if (a == 2)
                {
                    ucPoint.Location = new Point(220, 115);

                }
                else if (a == 3)
                {
                    ucPoint.Location = new Point(395, 115);

                }
                else if (a == 4)
                {
                    ucPoint.Location = new Point(50, 307);

                }
                else if (a == 5)
                {
                    ucPoint.Location = new Point(220, 307);

                }
                else if (a == 6)
                {
                    ucPoint.Location = new Point(395, 307);

                }
                string place = a.ToString();

                textBox1.Text = "0" + place;
                count = spoting(textBox1.Text, count);
        ...
        public void returnposition()
        {
            if (_IsInit)
            {
                ...
                //初始化支线程并开启
                _th = new System.Threading.Thread(new System.Threading.ThreadStart(LoopReceiveData1));
                _th.Start();
        ...
        private void LoopReceiveData1()
        {
            ...
            while (!_IsInit)
            {
                try
                {
                    if (count > 4)
                    {
                        if (MessageBox.Show("车辆停放成功！", "提示") == DialogResult.OK)
                        {
                            _th.Abort();
                            this.Close();
                        }
        ...
        public int spoting(string place, int count)
        {
            place = place + "        ";
